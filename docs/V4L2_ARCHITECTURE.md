...
│  │    └─ TCP → PICO 头显                                            │   │
│  │                                                                  │   │
│  │  输出 2: Raw Video YUYV422 (副输出, 降帧率)                      │   │
...
└─────────────────────────────────────────────────────────────────────────┘
                    │
                    │ Raw Video YUYV422 (内核内存传输,零拷贝)
                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  虚拟设备: /dev/video99 (v4l2loopback)                                  │
│  内核缓冲区: 保存最新的 YUYV422 帧 (无压缩)                              │
└─────────────────────────────────────────────────────────────────────────┘
                    │
                    │ Raw Video YUYV422 (读取)
                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
| 特性 | MJPEG 方案 (旧) | Raw Video YUYV422 方案 (新) |
|------|-----------------|----------------------------|
| 帧边界 | 不清晰,可能丢失 | 清晰,固定大小 |
| 丢帧率 | ~5-10% | **0%** |
| 带宽 | ~3 MB/s (压缩) | ~55 MB/s (未压缩) |
| CPU 编码 | 无 (MJPEG 已压缩) | 无 (直接传输) |
| CPU 解码 | 需要 (MJPEG→BGR) | 需要 (YUYV→BGR) |
| 推荐场景 | 带宽受限 | **零丢帧优先** |

**带宽计算 (2560x720@30fps)**:
- MJPEG (压缩率 ~30:1): 2560×720×3 / 30 ≈ 180 KB/帧 × 30 ≈ **5.4 MB/s**
- Raw YUYV422: 2560×720×2 = **3.7 MB/帧** × 30 ≈ **110 MB/s**

虽然 Raw Video 带宽增加约 20 倍，但在**内核内存中传输**，对系统性能影响可忽略。关键收益是**零丢帧**。
